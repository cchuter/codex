...
- The `$CODEX_HOME/config.toml` configuration file where the `CODEX_HOME` environment value defaults to `~/.osmiflow`. (Note `CODEX_HOME` will also be where logs and other Codex-related information are stored.)
...
To have Codex use this script for notifications, you would configure it via `notify` in `~/.osmiflow/config.toml` using the appropriate path to `notify.py` on your computer:

```toml
notify = ["python3", "/Users/mbolin/.osmiflow/notify.py"]
```
